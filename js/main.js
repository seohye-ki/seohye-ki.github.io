    ...
    if (viewMoreBtn) {
        viewMoreBtn.addEventListener('click', () => {
            ...
            if (hiddenReviews.length > 0) {
                ...
                viewMoreBtn.innerHTML = isHidden ? '접기 ↑' : '더보기 ↓'; // Improved Label
            }
        });
    }
    /* --- REVEAL ANIMATION --- */
    ...
    const reviewData = {
        "1": {
            ...
            content: `<h1>짧은 시간 안에 코칭모드 기능을 완성해낸 모습이 정말 인상 깊었어요</h1>
                        ...
                        <p>또 하나 기억에 남는 건 대본 수정 기능이었어요. 며칠간 혼자 끙끙대더니 어느새 문단 추가, 수정, 이동은 물론이고 제스처 연동까지 완벽하게 구현해냈더라고요. 보는 내내 "이건 진짜 앱이다!" 싶을 만큼 완성도가 뛰어났습니다.</p>
        ...
        "2": {
            ...
            content: `<h1>믿음직스럽고 뭐든지 척척 해내는 팀원!</h1>
                        <blockquote><strong>믿음직스럽고 뭐든지 척척 해내는 팀원!</strong></blockquote>
        ...
        "4": {
            ...
            content: `<h1>우리 프로젝트의 행동대장이었습니다.</h1>
                        <blockquote>한 단어로 요약하자면, 김서현 우리 프로젝트의 행동대장이었습니다.</blockquote>
        ...
        "5": {
            ...
            content: `<h1>기회가 된다면 또 함께하고 싶은 베스트 팀메이트</h1>
                        <blockquote>개발적인 부분에서의 동료평가</blockquote>
                        <ul>
                        ...
                        <p>총평?⭐️⭐️⭐️⭐️⭐️ 기회가 된다면 또 함께하고 싶은 베스트 팀메이트</p>
                        <p>서현님은 정말 짱이에요</p>`
        },
        "6": {
            name: "이O호", project: "DocshunD",
            content: `<h1>믿고 맡길 수 있는 동료</h1>
                        <blockquote>"책임감이 끝내주는, 믿고 맡길 수 있는 동료!"</blockquote>
                        <ul>
                        <li>동료와의 원활한 소통</li>
                        <li>주도적인 작업 태도</li>
                        ...
                        <p>함께 프로젝트를 진행하면서 김서현님은 주도적인 태도와 강한 책임감을 보여주었습니다. 김서현님은 자신에게 주어진 작업 이외에 항상 새로운 작업을 주도적으로 처리하여 프로젝트의 진행 속도를 크게 높여주었습니다. 또한, 로그인 및 채팅 관련 오류를 끝까지 분석하고 해결함으로써 서비스가 정상적으로 작동하는데 크게 기여하였습니다. 결론적으로 김서현님은 책임감이 강하며 팀의 작업 효율성을 높여주는 신뢰할 수 있는 동료라고 생각합니다.</p>`
        },
        "7": {
            name: "이O환", project: "Everymatch",
            content: `<h1>소통이 즐거운 프론트엔드 개발자님</h1>
                        <blockquote>“말 잘 들어주는 프론트님”이라는 한마디가 정말 딱 어울리는 팀원이었습니다.</blockquote>
        ...
        "8": {
            ...
            content: `<h1>새로운 기술과 아이디어를 도입하는 것을 두려워하지 않는 사람</h1>
                        <blockquote>새로움에 익숙한 사람 : 새로운 기술과 아이디어를 도입하는 것을 두려워하지 않는 사람<br>고민을 즐기는 사람 : 고민을 통해 문제를 해결하는 것을 즐기는 사람<br>같이 있으면 즐거운 사람 : 함께 웃으며 프로젝트를 진행하게 만드는 사람<br>솔직한 사람 : 부족한 부분이나 문제가 있으면 숨김없이 말하는 사람</blockquote>`
        },
        "9": {
            name: "익명", project: "42Seoul",
            content: `<h1>벽을 느껴도 해결책을 찾으려 하고, 결국엔 결과를 만들어낸다.</h1>
                        <blockquote>팀 분위기를 자연스럽게 살리는 편이다.<br>대화할 때 긍정적인 에너지를 주지만, 일부러 분위기를 띄우려 하지는 않는다.<br>가끔 예상치 못한 날카로운 질문을 던져서 고민하게 만들 때가 있는데, 결국 핵심을 짚고 있어서 피하기 어렵다. 어려운 상황에서도 쉽게 포기하지 않고 끝까지 해내려는 성향이 강하다. 벽을 느껴도 해결책을 찾으려 하고, 결국엔 결과를 만들어낸다.</blockquote>`
        },
        "10": {
            name: "이O준", project: "42Seoul",
            content: `<h1>서현님의 최대 강점은 ‘성장형 친화력’입니다.</h1>
                        ...
                        <blockquote>서현님의 최대 강점은 ‘친화력’이라고 생각합니다. 처음에 제게 먼저 말을 걸어주셨던 것도 서현님이셨고, 저뿐만 아니라 다양한 사람들과 붙임성 있게 이야기를 나누는 모습이 42 Seoul 내에서 네트워크를 형성하는데 큰 도움이 되었을 거라고 생각합니다. 또한 서현님은 비전공자 출신이지만, 그렇기에 더욱 코드 한 줄, 개념 하나를 허투루 생각하지 않으셨습니다. 42 Seoul에는 ‘동료 평가’라는, 자신이 완료한 과제를 무작위로 배정된 동료에게 평가받는 시스템이 있는데, 구현한 프로그램에 대해서 확실한 이해력을 바탕으로 이해하기 쉽게 설명해 주시는 모습이 아직도 기억에 남습니다. 앞으로 새로운 동료, 낯선 환경에서 다시 적응을 해야 한다 하더라도 제가 1년간 바라본 서현님이라면 충분히 적응해 나갈 수 있을 것이라고 확신합니다.</blockquote>`
        },
        "11": {
            name: "익명", project: "42Seoul",
            content: `<h1>진도가 느린 동료가 있어도 늘 옆에서 따뜻하고 불편하지 않게 격려하며 함께 이끌어갑니다</h1>
                        <p>서현님은 처음 봤을 때부터 느껴진 정말 밝고 긍정적인 에너지가 굉장히 인상깊은 분이었습니다. 낯선 이에게도 한결 같이 편하게 말을 나눠주셔서 누구나 편하게 가까워질 수 있는 분이었습니다. 이런 서현님만의 소프트스킬들은 동료학습을 하고 함께 팀 프로젝트를 진행할 때에도 긍정적으로 작용했습니다.</p>
                        ...
                        <p>함께 팀프로젝트를 할 때에도 서현님께서 굉장히 적극적으로 임하시고 소통해주셔서 온전히 몰입해 협업하는 즐거운 경험을 할 수 있었습니다. 덕분에 문제 상황을 마주했을 때에도 원활하게 소통하며 문제 해결 목적에 집중한 해결점을 찾을 수 있었습니다. 상대적으로 진도가 느린 동료가 있어도 늘 옆에서 따뜻하고 불편하지 않게 격려하며 함께 이끌어가시는 모습도 굉장히 인상깊었습니다.</p>
